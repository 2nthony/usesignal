...
export function useScrollLock(
  ...
  useWatch(element, (el) => {
    ...
    if (target) {
      ...
      if (ele.style.overflow === 'hidden') {
        return internalIsLock.value = true
      }

      if (internalIsLock.value) {
  ...
  const lock = () => {
    ...
    if (!el || internalIsLock.value) {
      return
    }

    el.style.overflow = 'hidden'
    internalIsLock.value = true
  }
  ...
  const unlock = () => {
    ...
    internalIsLock.value = false
  }

  useWatch(isLocked, (val) => {
    if (val) {
      lock()
    }
    else {
